...
namespace ortc
{
  namespace internal
  {
	...
	size_t ComputeHmac(MessageDigest* digest,
	  ...
	  boost::shared_ptr<uint8[]> new_key(new uint8[block_len]);
	  ...
	  boost::shared_ptr<uint8[]> o_pad(new uint8[block_len]), i_pad(new uint8[block_len]);
	  for (size_t i = 0; i < block_len; ++i) {
		o_pad[i] = 0x5c ^ new_key[i];
		i_pad[i] = 0x36 ^ new_key[i];
	  }
	  // Inner hash; hash the inner padding, and then the input buffer.
	  boost::shared_ptr<uint8[]> inner(new uint8[digest->Size()]);
