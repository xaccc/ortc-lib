...
namespace ortc
{
  namespace internal
  {
	...
	static EVP_PKEY* MakeKey() {
	  std::cout << "Making key pair"<< std::endl ;
	  ...
	  BN_free(exponent);
#endif
	  std::cout << "Returning key pair"<< std::endl ;
	  return pkey;
	}
	...
	static X509* MakeCertificate(EVP_PKEY* pkey, const SSLIdentityParams& params) {
	  ...
	  X509_NAME_free(name);
	  std::cout << "Returning certificate"<< std::endl ;
	  return x509;
	...
	static void PrintCert(X509* x509) {
	  ...
	  if (!temp_memory_bio) {
		std::cout << "Failed to allocate temporary memory bio"<< std::endl ;
		return;
	  }
	...
	OpenSSLIdentity* OpenSSLIdentity::GenerateInternal(
	  ...
	  std::cout << "Identity generation failed"<< std::endl ;
	  return NULL;
	}
	...
	SSLIdentity* OpenSSLIdentity::FromPEMStrings(
	  ...
	  boost::shared_ptr<OpenSSLCertificate> cert(
		  OpenSSLCertificate::FromPEMString(certificate));
	  if (!cert) {
		std::cout << "Failed to create OpenSSLCertificate from PEM string."<< std::endl ;
		return NULL;
	  }

	  BIO* bio = BIO_new_mem_buf(const_cast<char*>(private_key.c_str()), -1);
	  if (!bio) {
		std::cout << "Failed to create a new BIO buffer."<< std::endl ;
		return NULL;
	  }
	  (void)BIO_set_close(bio, BIO_NOCLOSE);
	  BIO_set_mem_eof_return(bio, 0);
	  ...
	  if (!pkey) {
		std::cout << "Failed to create the private key from PEM string."<< std::endl ;
		return NULL;
	  }

	  //return new OpenSSLIdentity(new OpenSSLKeyPair(pkey),
		//						 cert.release());
	  return new OpenSSLIdentity(new OpenSSLKeyPair(pkey),cert.get());
	}
